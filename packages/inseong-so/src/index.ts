...
class Constraints {
  static generate(
    ...
    if (resultingChecks) {
      ...
      result.moves = result.moves.filter(
        location => !resultingChecks({ piece, location, capture: false, moveIndex }).length,
      );
      result.captures = result.captures.filter(
        location => !resultingChecks({ piece, location, capture: true, moveIndex }).length,
      );
    }
    return result;
  }
  ...
  static runUntil(
    locationFunction: (integer: number, positions: PiecePositions) => PieceDirResponse,
    response: Options,
    args: ConstraintArguments,
  ) {
...
class View {
  ...
  drawPiecePositions(moves: Position[] = [], moveInner = '') {
