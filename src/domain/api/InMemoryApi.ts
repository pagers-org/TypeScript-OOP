...
export class InMemoryApi implements Api {
  getBeverages(): Beverage[] {
    ...
    ].map(item => new Beverage(item.id, item.name as BeverageName));
  }
  ...
  getMaterials(): Material[] {
    ...
    ].map(item => new Material(item.id, item.name as MaterialName));
  }
  ...
  getOptions(): Option[] {
    ...
    ].map(item => new Option(item.id, item.optionGroupId, item.name));
  }
