...
export class CafeOrderListRow extends Component {
  ...
  public static create(beverageName: string, order: Order) {
    const instance = document.createElement('cafe-order-list-row') as CafeOrderListRow;
  ...
  template() {
    const order = this.order;
    const beverageName = this.beverageName;
